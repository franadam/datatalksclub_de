...
This only runs models in models/staging/ and their dependencies
...
SELECT
    ...
    LAG(total_amount) OVER (PARTITION BY service_type, quarter ORDER BY year) AS prev_year_revenue,
        (total_amount - LAG(total_amount) OVER (PARTITION BY service_type, quarter ORDER BY year)) / 
    NULLIF(LAG(total_amount) OVER (PARTITION BY service_type, quarter ORDER BY year), 0) AS yoy_growth
